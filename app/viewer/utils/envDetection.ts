...
export function detectLocalEnvironment(): {
  ...
  if (typeof process !== 'undefined' && process.env) {
    ...
    console.log(`[环境检测] ENABLE_LOCAL_FS: ${process.env.ENABLE_LOCAL_FS || '(未设置)'}`);
    console.log(`[环境检测] FILES_DIRECTORY: ${process.env.FILES_DIRECTORY || './files (默认)'}`);
  }
  
  // 1. 环境变量（最可靠）✅
  const explicitEnabled = process.env.ENABLE_LOCAL_FS === 'true';
  const explicitDisabled = process.env.ENABLE_LOCAL_FS === 'false';
  const isDev = process.env.NODE_ENV === 'development';
  
  if (explicitEnabled) {
    score += 10;
    reasons.push('Explicitly enabled by ENABLE_LOCAL_FS=true');
  }
  if (explicitDisabled) {
    score = 0;
    reasons.push('Explicitly disabled by ENABLE_LOCAL_FS=false');
  ...
  if (typeof process !== 'undefined' && process.env) {
    ...
    console.log(`[环境检测] 判断结果: ${isLikelyLocal ? '✅ 可能是本地环境' : '❌ 可能不是本地环境'}`);
...
export function isLocalRequest(request: Request): {
  ...
  } else if (host.match(/^10\.\d+\.\d+\.\d+/) || host.match(/^172\.(1[6-9]|2[0-9]|3[0-1])\.\d+\.\d+/)) {
    score += 2;
  ...
  const confidence = 
    host.includes('localhost') || host.includes('127.0.0.1') ? 'high' :
    origin && (origin.includes('localhost') || origin.includes('127.0.0.1')) ? 'high' :
    score >= 5 ? 'medium' : 'low';
...
export function canAccessLocalFS(request?: Request): {
  ...
  if (process.env.ENABLE_LOCAL_FS === 'true') {
    const result = {
      ...
      reasons: ['Explicitly enabled by ENABLE_LOCAL_FS=true', ...envCheck.reasons]
    };
    if (typeof process !== 'undefined' && process.env) {
      console.log(`[访问控制] ✅ 允许访问: 显式启用 (ENABLE_LOCAL_FS=true)`);
    }
    return result;
  }
  
  if (process.env.ENABLE_LOCAL_FS === 'false') {
    const result = {
      allowed: false,
      confidence: 'high' as const,
      reasons: ['Explicitly disabled by ENABLE_LOCAL_FS=false', ...envCheck.reasons]
    };
    if (typeof process !== 'undefined' && process.env) {
      console.log(`[访问控制] ❌ 拒绝访问: 显式禁用 (ENABLE_LOCAL_FS=false)`);
    }
    return result;
  }
  ...
  if (request && isDev) {
    ...
    if (reqCheck.isLocal && (reqCheck.confidence === 'high' || reqCheck.confidence === 'medium')) {
      const result = {
        allowed: true,
        confidence: reqCheck.confidence === 'high' ? 'high' as const : 'medium' as const,
        reasons: [
      ...
      if (typeof process !== 'undefined' && process.env) {
        console.log(`[访问控制] ✅ 允许访问: 开发环境 + localhost 请求 (可信度 ${result.confidence})`);
      }
      return result;
    }
  }
  ...
  if (envCheck.isLikelyLocal) {
    ...
    if (typeof process !== 'undefined' && process.env) {
      console.log(`[访问控制] ✅ 允许访问: 环境检测通过 (可信度 ${result.confidence})`);
    }
    return result;
  }
